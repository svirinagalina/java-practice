...
Generics — это механизм параметризации типов, который позволяет создавать классы, интерфейсы и методы, работающие с разными типами данных, при этом сохраняя типобезопасность на этапе компиляции.
...
4) нельзя использовать instanceof с параметризованным типом
5) Нельзя создавать static поля 
...
Wildcard (?) — это подстановочный тип, обозначающий неизвестный параметр типа. Используется в сигнатурах методов, чтобы гибко ограничивать допустимые типы (через extends и super) при чтении или записи данных из обобщённых коллекций.

[🔝 Вернуться к оглавлению](#оглавление)

## Что такое принцип PECS?
Принцип PECS (Producer Extends, Consumer Super) помогает запомнить, как выбирать границы wildcard:

если коллекция производит (отдаёт) данные — extends;

если коллекция потребляет (принимает) данные — super.
...
ковариантность позволяет использовать подтипы вместо базового типа (? extends T) -- только чтение T + Object
писать ничего нельзя
контрвариативность -- позволяет использовать супертипы (? super T) только запись T и его потомков (наследников), читать только Object
...
LikedHashMap  это HashMAp, которая созраняет порядок добавления, использует двусвязный список для упорядочения и хеш-таблицу для быстрого доступа O(1)

[🔝 Вернуться к оглавлению](#оглавление)

## Что вы знаете о коллекциях типа LinkedHashSet?
это HashSet, который сохраняет порядок добавления элементов. Основан на LinkedHashMap, использует двусвязный список для порядка и хэш-таблицу для быстрого доступа O(1)
...
TreeSet это коллекция, хранящая уникальные элементы в отсортированном порядке. Основана на TreeMap, использует красно-черное дерево, операции выполняются за O(log n)

## Что вы знаете о коллекциях типа TreeMap?
TreeMap -- коллекция, хранящая пары ключ-значение в отсортированном порядке. Основана на красно-черном дереве, обеспечивает логарифмиеческое время операций O(log n)
...
HashMap преобразует связный список (LinkedList) внутри бакета в красно-чёрное дерево, если выполняются два условия:

В одном бакете накопилось 8 или более элементов (пороговое значение TREEIFY_THRESHOLD = 8).

Общий размер таблицы (capacity) больше 64.
Если размер меньше, вместо преобразования происходит расширение таблицы (resize), чтобы уменьшить количество коллизий.

📊 Это сделано для оптимизации:

пока элементов мало — используется связный список (простая и быстрая структура);

когда элементов много — дерево ускоряет поиск и вставку с O(n) → O(log n).

Если после удаления элементов в бакете их количество падает ниже 6 (UNTREEIFY_THRESHOLD = 6), структура обратно превращается в связный список.


## В чем разница между интерфейсами Comparable и Comparator?

Comparable — задаёт естественный порядок прямо в классе.

Comparator — задаёт внешний порядок, который можно легко менять.

Класс может реализовывать Comparable, но при этом ты можешь создавать сколько угодно Comparator для разных вариантов сортировки.

[🔝 Вернуться к оглавлению](#оглавление)

## Что возвращает Comparator и Comparable?

Comparable и Comparator возвращают целое число (int), которое показывает результат сравнения двух объектов.


compareTo() (из Comparable) и compare() (из Comparator) возвращают:
...
Через for-each — самый простой способ:
for (String s : list) { ... }

Через обычный for с индексом — если нужен доступ по индексу:
for (int i = 0; i < list.size(); i++) { ... }

Через Iterator — безопасно удалять элементы во время итерации:
