...
## В Java сигнатурой метода называется:

👉 имя метода + список типов параметров (их порядок и количество)
...
статический (перегрузка) и динамический  (переопределение)
...
в метод попадает копия ссылки на объект.
Это значит:

сам объект можно изменять (его поля, содержимое массива, список и т.д.),

но переназначить ссылку (чтобы «заменить» объект) снаружи метода нельзя.

[⬆ Вернуться к содержанию](#содержание)

## Что такое нативные методы?

Нативные методы — это методы Java-класса, которые реализованы не на Java, а на другом языке (чаще всего C или C++).
В коде они объявляются с помощью ключевого слова native, а их реализация подключается через JNI (Java Native Interface).

Плюсы:

✅ Доступ к системным ресурсам (низкоуровневое управление памятью, процессами, устройствами).

✅ Возможность использовать готовые, проверенные библиотеки на C/C++.

✅ Иногда — более высокая производительность (например, в обработке изображений или математических вычислениях).

Минусы:

❌ Потеря кроссплатформенности (код становится привязан к конкретной ОС и архитектуре).

❌ Сложность разработки и отладки (работа с JNI требует знаний C/C++).

❌ Риск ошибок памяти (утечки, сегфолты) — в отличие от чистой Java, где за память отвечает GC.

❌ Безопасность: нативный код может обойти sandbox и сделать что угодно.
...
Создать папку с таким же именем в файловой системе:

mypackage/Person.java

Доступ к классу из другого пакета через import:
...
Конструктор в Java — это специальный метод, который вызывается при создании объекта с помощью new.
Он нужен, чтобы инициализировать поля объекта.
...
Конструктор по умолчанию — это конструктор без параметров, который компилятор автоматически создаёт,
если программист не написал ни одного конструктора.
...
Да, у абстрактного класса может быть конструктор.

Он нужен для инициализации полей и логики, общей для всех наследников.

При создании объекта наследника сначала вызывается конструктор абстрактного класса (super(...)), а потом уже — его собственный.

[⬆ Вернуться к содержанию](#содержание)

## Будет ли в классе конструктор и какой, если при его создании ни один не был задан

Если ты не написала ни одного конструктора в классе,
компилятор автоматически создаст конструктор без параметров:
...
вызывает super() (конструктор родительского класса без параметров),

имеет ту же область видимости, что и сам класс.

[⬆ Вернуться к содержанию](#содержание)

## Зачем this в конструкторе абстрактного класса

для того же, для чего и в обычном конструкторе, но он будет фактически работать с экземпляром наследника
...
Используется ключевое слово this(...) с аргументами.

Такой вызов должен быть первой строкой конструктора.
...
абстрактный класс — через abstract class.

Наследование:

класс может имплементировать несколько интерфейсов,

но наследовать только один абстрактный класс.

Поля:

в интерфейсе все поля по умолчанию public static final (константы),

в абстрактном классе можно любые (обычные, private, protected, static, и т.д.).

Методы:

в интерфейсе методы без тела → по умолчанию public abstract (с Java 8 могут быть ещё default и static),

в абстрактном классе методы могут быть как абстрактные, так и с реализацией.

Конструкторы:

у интерфейса нет конструкторов,

у абстрактного класса конструкторы могут быть.

##  Какие методы в интерфейсе могут иметь реализацию

В интерфейсе могут иметь реализацию:

default-методы (с Java 8) — обычные методы с телом, можно переопределять в классах.

static-методы (с Java 8) — привязаны к интерфейсу, вызываются через InterfaceName.method().

private-методы (с Java 9) — вспомогательные, доступны только внутри интерфейса.

Все остальные методы по умолчанию остаются абстрактными.
