...
Класс может extends только один класс (в том числе абстрактный).

Но может implements много интерфейсов.

Интерфейс может extends несколько интерфейсов.

В интерфейсе поля всегда public static final (константы).

В абстрактном классе могут быть любые поля: private, protected, изменяемые, не статические.

Интерфейс: методы по умолчанию абстрактные, но с Java 8 могут быть default и static, а с Java 9 — ещё и private.

Абстрактный класс: может содержать обычные методы, абстрактные, static, final.

Интерфейс не может иметь конструкторов.

Абстрактный класс может иметь конструкторы (используются для инициализации полей при создании наследников).

## Как сделать из обычного класса абстрактный класс, не используя abstract?

Можно сделать конструктор private (или все конструкторы private).

Тогда создать объект напрямую нельзя, но класс можно использовать как утилитарный (через static методы).
