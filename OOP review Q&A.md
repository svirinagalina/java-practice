...
Раннее (early binding, compile-time) → компилятор заранее знает, какой метод вызвать (например, перегрузка методов).

Позднее (late binding, runtime) → окончательный метод выбирается во время выполнения, в зависимости от реального типа объекта (например, переопределение методов в наследниках).

## Varargs (аргументы переменной длины)

апись: void method(String... args).

В метод можно передавать любое количество аргументов или массив.

Ограничения:

varargs-параметр может быть только один;
