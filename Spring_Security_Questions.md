...
- [Способы добавить секьюрность к контроллеру](#способы-дdобавить-секьюрность-к-контроллеру)
...
Principal, Authorities, Authentication
Principal — это текущий пользователь, его личность в системе; в Spring Security обычно это UserDetails или имя пользователя внутри Authentication.getPrincipal().
​

Authorities — это список прав/ролей пользователя (например, ROLE_USER, ROLE_ADMIN), который хранится как коллекция GrantedAuthority внутри Authentication.
​

Authentication — это объект, который описывает текущего пользователя: кто он (principal), чем подтвердился (credentials), какие у него права (authorities) и аутентифицирован ли он (isAuthenticated
...
Basic Authentication — это простой способ защиты эндпоинта: сначала клиент стучится без логина/пароля, сервер говорит 401 Unauthorized и в заголовке WWW-Authenticate: Basic показывает «мне нужны креды», после этого клиент начинает слать в каждом запросе заголовок Authorization: Basic <base64(login:password)>, Spring Security достаёт оттуда логин/пароль, лезет в базу (хоть через DriverManager.getConnection), сравнивает введённый пароль с захешированным (обычно BCrypt) и, если всё совпало, пускает к ресурсу, а если нет — снова отвечает 401, то есть «ты не прошёл аутентификацию»

плюсы: простая реализация, встроенная поддержка во всех браузерах/клиентах, удобно для внутренних сервисов, теста и временной защиты при наличии HTTPS.

Нельзя «выйти из системы» на сервере: браузер продолжает слать заголовок, пока его не закроешь/не сбросишь кэш, а кража одного запроса даёт злоумышленнику постоянный доступ.
...
Односторонняя связь (unidirectional) — это когда один класс «знает» о другом и держит ссылку/коллекцию, а в обратную сторону ссылки нет: например, User имеет List<Address>, но Address не содержит поля user. Двусторонняя связь (bidirectional) — когда обе сущности знают друг о друге: User хранит List<Address>, а Address хранит User user, и в JPA/Hibernate такая связь настраивается через пару аннотаций (@OneToMany / @ManyToOne, mappedBy) с определением владеющей стороны
​
...
​​
N+1 проблема — это когда Hibernate/JPA делает один запрос в БД, чтобы загрузить список родительских сущностей (например, всех пользователей), а потом для каждого из N элементов делает ещё один отдельный запрос, чтобы подгрузить связанные данные (например, их адреса или заказы), и в итоге вместо одного-двух SQL-запросов уходит 1 + N запросов, что убивает производительность; решается через JOIN FETCH, @EntityGraph или настройку FetchType.EAGER там, где нужно, чтобы всё нужное загружалось одним запросом.
...
Клиент и сервер чётко разделены: клиент отвечает за UI и опыт пользователя, сервер — за данные и бизнес‑логику. Это упрощает масштабирование и независимое развитие фронта и бэка.

Отсутствие состояния (Stateless)
Сервер не хранит состояние сессии между запросами; каждый запрос содержит всю необходимую информацию (токен, параметры и т.д.), чтобы его можно было обработать независимо.

Кэширование (Cacheable)
Ответы сервера должны явно указывать, можно ли их кэшировать и на сколько, чтобы уменьшить количество запросов и повысить производительность (заголовки Cache-Control, ETag и т.п.).

Единообразие интерфейса (Uniform Interface)
Единые соглашения: использование стандартных HTTP‑методов (GET, POST, PUT, DELETE, PATCH), понятные URI ресурсов, предсказуемые коды ответа и форматы данных. Это снижает связность и упрощает использование API.

Многоуровневая система (Layered System)
Между клиентом и конечным сервером могут быть прокси, балансировщики, шлюзы, кэши и т.д., причём клиент не знает о количестве слоёв; каждый уровень отвечает за свою задачу.

Код по требованию (Code on Demand, опционально)
Сервер может по запросу отправлять исполняемый код (например, скрипты), который расширяет функциональность клиента. Этот принцип единственный необязательный, остальные пять — обязательны для RESTful‑архитектуры.
...
AJAX технология — старый общий термин/подход: JS создаёт XMLHttpRequest, отправляет HTTP‑запрос в фоне, получает ответ и частично обновляет страницу.
​

fetch реализация — современный встроенный метод fetch(url, options), который делает те же асинхронные HTTP‑запросы, но с более простым и удобным API на промисах (then/await).
...
Минусы. Много лишнего CSS/JS (блоат), сайты часто похожи друг на друга, HTML засорён классами, ради уникального 

## Какой путь проходят данные от страницы авторизации до места назначения

Путь данных от страницы авторизации в Spring Boot + Spring Security:

Форма логина (HTML) → пользователь вводит логин/пароль → браузер шлёт HTTP POST на `/login` → запрос попадает в `FilterChainProxy` → `UsernamePasswordAuthenticationFilter` вытаскивает креды → передаёт их в `AuthenticationManager` → тот дергает `AuthenticationProvider`, который через `UserDetailsService` грузит пользователя из БД, сравнивает пароли через `PasswordEncoder` (например, BCrypt) → при успехе создаётся `Authentication` с пользователем и его ролями, он кладётся в `SecurityContext`, который привязывается к HTTP‑сессии → `AuthenticationSuccessHandler` редиректит на нужную страницу (`/user`, `/admin` и т.п.) → дальше запросы к защищённым контроллерам идут уже с установленной аутентификацией, а доступ контролируется по ролям (`@PreAuthorize`, `hasRole` и т.д.).

## В чем разница между UserDetails и UserDetailsService

**UserDetails** — это интерфейс‑обёртка над твоей сущностью `User`: объект с данными пользователя для Spring Security (username, зашифрованный пароль, роли/authorities, флаги enabled/accountNonLocked и т.п.). **UserDetailsService** — это сервис с одним методом `UserDetails loadUserByUsername(String username)`, который по логину лезет в БД/LDAP/внешний сервис, находит пользователя и возвращает `UserDetails`, чтобы Spring Security мог проверить пароль и построить `Authentication`.дизайна приходится активно всё переопределять.
​
