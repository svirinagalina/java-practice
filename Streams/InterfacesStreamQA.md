...
Функциональное программирование стоит использовать там, где важнее выразить 
что нужно сделать с данными, а не как именно по шагам. Оно особенно полезно для сложных
преобразований коллекций и потоков данных, а также когда требуется безопасный параллелизм.

Когда полезно функциональное программирование
ФП описывает вычисления через чистые функции: они не зависят от внешнего изменяемого состояния 
и не имеют побочных эффектов (не пишут в поля, БД, логгер и т.п.). Это упрощает тестирование 
(достаточно подать вход и проверить выход) и делает поведение кода предсказуемым.​
Такой стиль хорошо подходит для задач вида «взять коллекцию, отфильтровать, трансформировать,

агрегировать», обработки событий, работы с потоками данных и реактивных сценариев,
где конвейер операций важнее конкретных циклов и мутаций.​

Отличия от ООП/процедурного стиля и выбор подхода
В ООП и процедурном стиле основной инструмент — изменяемые объекты и последовательные 
инструкции: создать список, в цикле менять его, обновлять счётчики и т.п.
Это естественно для бизнес‑логики с большим количеством состояний, CRUD‑операций и сложного 
жизненного цикла сущностей.​
ФП удобнее, когда: много однотипных преобразований коллекций, 
нужно легко параллелить вычисления, важно минимизировать состояние и синхронизацию;
ООП/процедурный стиль предпочтительнее, когда доминируют долгоживущие сущности с богатым 
поведением и сложным изменяемым состоянием (доменные модели, UI‑компоненты, игры и т.п.).
...
к которому можно привязать лямбду или метод‑референс. Примеры: Runnable, Callable, Comparator, Supplier, 
Consumer, Function, Predicate и их примитивные варианты.
...
Лямбда‑выражение — краткая форма записи реализации функционального интерфейса в виде (args) -> тело. 
Тело может быть как одной строкой‑выражением, так и блоком кода в фигурных скобках с несколькими операторами 
и return.
Лямбда представляет собой «кусок поведения» (функцию), который можно передать как значение: в метод, 
сохранить в переменную, вернуть из метода.​

В Java каждая лямбда всегда привязана к какому‑то функциональному интерфейсу и реализует 
его единственный абстрактный метод (SAM).​
...
То есть лямбда вполне может быть в несколько строк: если нужно больше логики,
используют фигурные скобки и явный return (кроме случаев, когда возвращать
нечего — тогда достаточно просто операторов)
...
Заменить на лямбду можно только анонимный класс, реализующий функциональный 
интерфейс и не использующий специфические возможности класса 
(то есть свои поля и состояние: в анонимном классе можно объявить поля,
хранить в них данные, изменять их; у лямбды своих полей 
и собственного состояния нет, она только реализует один метод интерфейса.​

Несколько методов и особое this/super: анонимный класс может определять 
дополнительные методы, обращаться к this как к экземпляру этого класса и к super. 
Лямбда такого отдельного «своего» this не имеет
— this внутри лямбды относится к внешнему классу, и дополнительных методов добавить нельзя.​

То есть заменить на лямбду можно только тот анонимный класс, 
который по сути просто реализует один метод интерфейса, 
без собственного состояния и доп. методов.)

(например, собственные поля, несколько методов, нестандартное использование this/super). 
В общем случае не любой анонимный класс заменяем лямбдой.
...
Метод‑референс — компактная ссылка на уже существующий метод или конструктор вместо лямбды. 
Записывается как Class::staticMethod, obj::instanceMethod или Class::new
...
Comparable даёт один встроенный естественный порядок у самого класса.
Comparator позволяет создавать несколько независимых стратегий сравнения снаружи (по имени, дате, цене и т.д.).

## 18. Что такое стримы? Для чего нужны? Когда лучше использовать

Стримы — это последовательность элементов с поддержкой декларативных операций вроде фильтрации, 
маппинга и агрегации. Их удобно использовать, когда нужно построить конвейер
преобразований данных без явных циклов и ручного управления коллекциями.
...
или выполняют побочный эффект. Примеры: forEach, collect, reduce, count, anyMatch, allMatch, noneMatch,
findFirst, findAny.
...
flatMap берёт элемент, превращает его в поток или коллекцию, а затем разворачивает все полученные 
потоки в один плоский стрим.
...
peek — промежуточная операция для «подглядывания» в поток (например, логирования), 
а forEach — терминальная операция, которая запускает выполнение конвейера.

## 26. Можно ли вызвать 2 терминальные операции? Может ли стрим использоваться повторно? Что будет без терминальной

Один и тот же стрим нельзя использовать повторно: после первой терминальной операции 
он считается потреблённым. Вызов второй терминальной операции на том же стриме приведёт к ошибке; 
чтобы выполнить ещё одну операцию, нужно создать новый стрим. Без терминальной операции конвейер не выполняется.
