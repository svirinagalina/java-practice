...
Функциональное программирование удобно, когда нужно работать с преобразованием данных — коллекциями, потоками,
реактивными цепочками. Оно опирается на чистые функции и неизменяемые данные, 
что делает код предсказуемым и хорошо подходящим для параллельной обработки. 
В отличие от ООП, где логика распределена между объектами и их состоянием,
функциональный стиль позволяет декларативно описывать, что нужно сделать, а не прописывать 
пошаговый алгоритм. В Java функциональный подход активно используется в Stream API,
реактивных фреймворках и обработке больших наборов данных.

Функция возвращает значение, которое можно использовать в выражениях, в то время как процедура только выполняет действия без возврата результата.
